  ...
  return (
    <section id="portfolio" className="py-20 bg-card relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects?.slice(0,3).map((project, index) => (
            ...
            >
              ...
              {/* Stats Badge (optional, using results or deliverables) */}
              {project.results && (
                <div className={`absolute top-4 left-4 z-20 transition-all duration-500 ${
                  hoveredIndex === index ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0'
                }`} style={{ transitionDelay: '100ms' }}>
                ...
                {project.projectUrl && (
                  <div className={`transition-all duration-500 ${
                    ...
                    <a
                      ...
                      className="flex items-center gap-2 text-primary bg-white px-4 py-2 rounded-full font-semibold text-sm hover:bg-primary hover:text-white transition-colors duration-300 shadow-lg"
                    >
                      View Project
                      <svg className="w-4 h-4 transform group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
      ...
      <style jsx>{`
        ...
        @keyframes float { 0%,100%{transform:translateY(0) translateX(0) rotate(0deg);} 33%{transform:translateY(-30px) translateX(30px) rotate(5deg);} 66%{transform:translateY(30px) translateX(-30px) rotate(-5deg);} }
        @keyframes scroll-fade-in { from {opacity:0;transform:translateY(40px);} to{opacity:1;transform:translateY(0);} }
        @keyframes scroll-scale-in { from {opacity:0;transform:scale(0.95) translateY(20px);} to {opacity:1;transform:scale(1) translateY(0);} }
        .scroll-fade-in { animation: scroll-fade-in 0.8s ease-out forwards; }
        .scroll-scale-in { animation: scroll-scale-in 0.7s ease-out forwards; opacity:0; }
      `}</style>
    </section>
  )
}
